...
if (isset($_POST['status_post'])) {
    ...
    if (mysql_query("INSERT INTO status_post(Status_text,Status_type,Status_time,User_id,hashtag) VALUES('$status_post_text','$status_post_type','$status_post_time','$user_id','$hashtag')")) {
        ?>         
        <script>
...
$db = new mysqli("localhost", "myparkco_my_park", "+DE[Gu^z}ctW?QnAX6", "myparkco_mypark");
$num_rows = mysql_query("SELECT * FROM messages WHERE Receiver_id = '$user_id' AND Message_status = 'unread' ");
if (mysql_num_rows($num_rows) > 0) {
...
//count followers
$db = new mysqli("localhost", "myparkco_my_park", "+DE[Gu^z}ctW?QnAX6", "myparkco_mypark");
$num_rows = mysql_query("SELECT * FROM followed WHERE followed_fld_user_id = '$user_id'");
if (mysql_num_rows($num_rows) > 0) {
