...
$more_results_clicked = $_SESSION['more_results_clicked_followers'];
...
$db = new mysqli("localhost", "myparkco_my_park", "+DE[Gu^z}ctW?QnAX6", "myparkco_mypark");
$user_list0 = "SELECT DISTINCT followed_fld_user_id,followed_flg_user_id FROM followed WHERE followed_fld_user_id = '$IN_user_id' ";

$response0 = $db->query($user_list0);
while ($row = $response0->fetch_assoc()) {
$following_user_id =$row['followed_flg_user_id'];

//show followers accounts

$db = new mysqli("localhost", "myparkco_my_park", "+DE[Gu^z}ctW?QnAX6", "myparkco_mypark");
$user_list = "SELECT * FROM user_details WHERE user_id = '$following_user_id' ";
...
//check page
if($_SESSION['more_results_clicked_followers'] == 0){
...
if (isset($_POST['show_older_posts'])) {
    ...
    $_SESSION['end'] = $end;
    $_SESSION['more_results_clicked_followers'] = 1;
